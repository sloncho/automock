...
ProxyquireWrapper.prototype.load = function (request, stubs, params) {
    ...
    this._stubs = {};   // TODO: rename to _mocks?
    this._params = params;
    ...
    original.__stubs__ = this._stubs;  // TODO: rename to __mocks__?
    delete this._stubs;
...
ProxyquireWrapper.prototype._require = function(callingModule, stubs, path) {
    // console.log('ProxyquireWrapper\'s _require override!');
    ...
    //   * it is being required directly by the file under test (?)v????? actually, maybe not!
    if (callingModule.filename === this._primaryModuleFile) {
        if (!this._stubs.hasOwnProperty(path)) {
            // // TODO: create a variation of _params that has just the needed
            // // passThru values?
            // var prefix = path + '.';
            // var localParams = {
            //     passThru: _.chain(this._params.passThru)
            //                 .filter(_.partial(_.startsWith, _, prefix))
            //                 .map(function(s) {
